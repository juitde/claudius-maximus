...
// equivalent common enough to check the same way. On other platforms this
// simply contributes no check, rather than a row that says nothing useful.
//
...
func checkSystemSleep() (CheckResult, bool) {
	...
	if minutes == 0 {
		...
		return CheckResult{Name: "system sleep", Status: StatusOK, Detail: detail}, true
	}
	return CheckResult{
		Name: "system sleep", Status: StatusWarn,
		Detail: fmt.Sprintf(
